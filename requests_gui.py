...
class RequestHandler:
    ...
    def create_request_box(self, request, i, parent=None):
        ...
        with dpg.table_row(tag=f'row{i}') if parent is None else dpg.table_row(tag=f'row{i}', parent=parent) as row:
            with dpg.table_cell():
                ...
                with dpg.collapsing_header(label=f'{source} to {dest}'):
    ...
    def create_request_table(self, parent=None):
        with dpg.table(header_row=False, tag='request_table', scrollY=True, pad_outerX=True) if parent is None else dpg.table(header_row=False, tag='request_table', scrollY=True, borders_innerH=True, parent=parent):#, row_background=True):
            # for _ in range(2):
    ...
    def start_requests(self):
        ...
        with dpg.window(label="Requests Display", height=300, width=600, tag='request_window'):
            self.create_request_table()
        with dpg.window(height=70, width=100, pos=(0, 300), no_close=True):
            dpg.add_button(label='Submit', tag='button1', callback=self.submit_callback)
            # dpg.bind_item_theme(dpg.last_item(), 'button_theme')
            set_font('button1', 15)
        with dpg.window(label='Path Display', height=200, width=200, pos=(600, 0), max_size=[1000, 300], no_close=True, autosize=True):
            ...
            with dpg.table(header_row=False):
                ...
                for idx in range(N_OPTIONS):
                    with dpg.table_row(tag=f'row_{idx}'):
                        with dpg.table_cell():
                            with dpg.collapsing_header(label=f'Path {idx+1}', tag=f'pathdisplay{idx}', show=False):
                                with dpg.group(tag=f'pathdet{idx}'):
                                    dpg.add_text('', tag=f'pathdetailed{idx}')
                                    dpg.add_button(label='Select this path')
                                # set_font(f'pathdisplay{idx}', 18)
                                set_font(f'pathdet{idx}', 15)
